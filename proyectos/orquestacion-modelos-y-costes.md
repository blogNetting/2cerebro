...
- Búsqueda agregada (sin fuente primaria única confirmada): DeepSeek-V4-Pro-Max 80.6% en un ranking de terceros (llm-stats.com / benchlm.ai), cifra que no coincide con la de Fireworks (95.2%) — **contradicción entre fuentes, sin resolver**; no hay acceso directo al leaderboard oficial `swebench.com` porque la tabla se renderiza vía JS y el fetch no la captura. [swebench.com](https://www.swebench.com/) (consultado, sin datos extraíbles) — sin verificar la cifra exacta de terceros.
...
3. **Contradicción sin resolver**: SWE-bench Verified de DeepSeek V4 Pro-Max varía entre 80.6% (agregador) y 95.2% (Fireworks, vendor) — posible diferencia de metodología (harness, nº de intentos, variante "Max" vs base) no aclarada por ninguna fuente consultada.
