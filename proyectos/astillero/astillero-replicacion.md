...
1. **Astillero es un repo real**, no solo el documento del wiki. Contiene: `.github/workflows/revisar.yml`, `reconciliar.yml` y `reproducir.yml` (reusable), `shared/implementar-core.md` y `rehacer-core.md` (importables por gh-aw), y la plantilla de copier (`CODEOWNERS` con usuario real, `AGENTS.md`, `docs/contrato-tarea.md`, y en `.github/workflows/`: `implementar`, `rehacer`, `revisar`, `reconciliar`, `ci`, `deploy`, `reproducir-bug` — completo desde el 2026-09-26, antes faltaban 4 de esas piezas).
