...
1. **Astillero es un repo real**, no solo el documento del wiki. Contiene: `.github/workflows/revisar.yml` y `reconciliar.yml` (reusable), `shared/implementar-core.md` y `rehacer-core.md` (importables por gh-aw), y la plantilla de copier (`CODEOWNERS` base, esqueleto de `AGENTS.md`, labels, plantilla de contrato de tarea de [[flujo-agentes-arquitectura]] §6).
