...
> **K1 es también donde vive la decisión de arquitectura — stack, monolito/microservicios, hexagonal, front/back —, no un paso aparte.** Verificado el 2026-09-26 contra la documentación oficial de Spec Kit (el framework con más adopción de los evaluados, 138.825★): *«[/speckit-plan] Generates the design artifacts from the spec. This is where implementation detail belongs — provide your tech stack and architecture»* ✔︎ — y esa decisión se valida contra `constitution.md` (los principios del proyecto), no se asume por defecto: en su propio ejemplo (Taskify), el plan elige *".NET Aspire with Postgres... Blazor Server"* justificado contra una constitución que exige *"microservices architecture"*. No hace falta un rol de arquitecto nuevo ni una ceremonia aparte (ADR formal, ronda de revisión propia) — encaja en lo que ya existe: el Diseñador decide la arquitectura al escribir `plan.md`, tú la apruebas en K2 igual que el resto de la especificación. Si la decisión es lo bastante grande, se registra además en `decisiones-producto.md` ([[capa-producto]] §6).
...
**Tests intactos: no es una puerta de CI, ya lo cubre `CODEOWNERS`** — corregido al implementar (2026-09-26): un script de CI que compruebe si el diff toca tests existentes sería redundante con lo que `CODEOWNERS` (más abajo) ya exige de forma nativa — `tests/**`, `**/test_*.py` y `**/*.test.*` requieren tu aprobación en el ruleset, GitHub lo bloquea solo sin necesitar un job aparte.
...
**Implementado de verdad el 2026-09-26, no solo diseñado** — `template/.github/workflows/ci.yml.jinja` en `blogNetting/astillero`, con las acciones de terceros fijadas por SHA real (verificado contra la API de GitHub el mismo día: `actions/checkout@3d3c42e`, `actions/setup-python@5fda3b9`, `actions/setup-node@8207627`, `codecov/codecov-action@303a32d`, `gitleaks/gitleaks-action@e0c47f4`, `google/osv-scanner-action@a345acf`). Probado generando un proyecto real con `copier` en python y en node: YAML válido en los dos, sin mezclar con código fuente. Opengrep queda fuera de la tabla por ahora — Semgrep CE ya cubre el caso de uso sin fricción de licencia a este nivel de adopción; revisar si se necesita reusar/redistribuir reglas.
