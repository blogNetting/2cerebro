...
- **Estado (2026-09-26):** arrancado con la skill `astillero-proyecto` y completo. Etiquetas de estado creadas; bootstrap de copier aplicado y fijado a **Astillero v0.1.0** (no `@main`: actualizaciones controladas, no automáticas) — `AGENTS.md`, `CODEOWNERS` con `@blogNetting`, `docs/contrato-tarea.md`, workflows de ejecutor/rehacer/revisor/reconciliador/CI/despliegue, todos compilados con `gh aw compile --approve` tras verificar que el único secreto nuevo (`DEEPSEEK_API_KEY`) coincide con el diseño. Ecosistema **Python** (solo puertas de CI y red del ejecutor, no es el framework de la app). Secretos `DEEPSEEK_API_KEY` y `CLAUDE_CODE_OAUTH_TOKEN` configurados por el usuario. Sin código de aplicación todavía — sigue en toma de requisitos (ver «Preguntas abiertas» arriba). Nada pendiente del arranque; el siguiente paso es la entrevista de diseño formal (K1) cuando el usuario lo decida.
